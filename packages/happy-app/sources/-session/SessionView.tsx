...
import { machineSpawnNewSession, sessionAbort, sessionArchive, sessionGoalAction, sessionKill, sessionSetAgentModes } from '@/sync/ops';
import { useHappyAction } from '@/hooks/useHappyAction';
import { HappyError } from '@/utils/errors';
import { storage, useIsDataReady, useLocalSetting, useRealtimeStatus, useSessionGitStatus, useSessionMessages, useSessionUsage, useSetting, useSocketStatus } from '@/sync/storage';
...
function SessionViewLoaded({ sessionId, session }: { sessionId: string, session: Session }) {
    ...
    const [, restartSession] = useHappyAction(async () => {
        ...
        if (session.permissionMode || session.modelMode) {
            storage.getState().updateSessionAgentModes(spawnResult.sessionId, {
                ...(session.permissionMode ? { permissionMode: session.permissionMode } : {}),
                ...(session.modelMode ? { modelMode: session.modelMode } : {}),
            });
        }
