...
describe('PermissionHandler', () => {
    it('auto-approves tool calls in yolo mode without surfacing a request', async () => {
        ...
        const result = await handler.handleToolCall(
            ...
            { command: 'pwd' },
            mode,
            { signal: controller.signal, requestId: 'req_toolu_yolo', toolUseID: 'toolu_yolo' },
        );
    ...
    it('auto-approves tool calls in bypassPermissions mode', async () => {
        ...
        const result = await handler.handleToolCall(
            ...
            mode,
            { signal: controller.signal, requestId: 'req_toolu_bypass', toolUseID: 'toolu_bypass' },
        );
    ...
    it('keeps main-thread request IDs unchanged', async () => {
        ...
        const pending = handler.handleToolCall(
            ...
            { command: 'pwd' },
            mode,
            { signal: controller.signal, requestId: 'req_toolu_main', toolUseID: 'toolu_main' },
        );
    ...
    it('uses agentID to disambiguate sub-agent permission requests with the same toolUseID', async () => {
        ...
        const firstPending = handler.handleToolCall(
            ...
            { command: 'pwd' },
            mode,
            { signal: firstController.signal, requestId: 'req_toolu_shared', toolUseID: 'toolu_shared', agentID: 'agent-a' },
        );
        const secondPending = handler.handleToolCall(
            'Bash',
            { command: 'whoami' },
            mode,
            { signal: secondController.signal, requestId: 'req_toolu_shared', toolUseID: 'toolu_shared', agentID: 'agent-b' },
        );
    ...
    it('can look up a single sub-agent response by raw toolUseID for transcript follow-up paths', async () => {
        ...
        const pending = handler.handleToolCall(
            ...
            { command: 'pwd' },
            mode,
            { signal: controller.signal, requestId: 'req_toolu_result', toolUseID: 'toolu_result', agentID: 'agent-a' },
        );
